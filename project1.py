...
class Cake():
    ...
    def generate_level(self, level_width, level_height):
        level_lift = self.level*self.level_height
        lvl_name = cmds.polyCylinder(height=level_height,
                                     radius=level_width)[0]
        cmds.xform(lvl_name, pivots=[0, -self.level_height/2, 0])
        cmds.xform(lvl_name, translation=[0, self.level_height, 0])
        cmds.xform(lvl_name, translation=[0, level_lift+(0.5*self.level_height), 0])
...
# I can call this in another function, generate_cake.
    def generate_cake(self):
        ...
        for level in range(0, self.levels):
            level_width = Cake.calculate_width()
            lvl_name = Cake.generate_level(self, level_width, level_height)
            cake_list.append(lvl_name)
            if self.icing is True:
                icing_name = Cake.generate_icing()
                cake_list.append(icing_name)
        grp_name = cmds.group(cake_list, name='cake_GRP')
        cmds.xform(grp_name, pivots=[0, 0, 0])
        if cake_color != "white":
            Cake.color_cake(grp_name, cake_color)
